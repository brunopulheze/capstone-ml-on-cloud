...
All changes live in **`notebooks/01-compare-models.ipynb`, Cell 7 (Holdout evaluation)**. They are **purely additive** — no existing functionality was removed and downstream cells (walk-forward, plots, naive comparison, model selection) still work unchanged.
...
Running the upgraded Cell 7 on the current data:
...
4. **The MLOps pipeline is unaffected.** Docker, FastAPI, Oracle Cloud, MLflow tracking, drift detection, GitHub Actions retraining, Next.js dashboard — every piece of the deployment work is still 100% valid. The deployment skill is the real point of this capstone, not model accuracy on a fundamentally hard problem.
...
| New helper functions | Same notebook → next code cell (Cell 7: Holdout evaluation) → `metrics_logret`, `directional_accuracy`, `unscale_logret` |
| Persistence baseline injection | Same cell → `all_preds["Persistence"] = (np.zeros_like(r_true), close_prev_test.copy())` |
| New printed tables | Same cell → `print(holdout_full_df.to_string())` and `print(logret_df.to_string())` blocks |
| Headline GRU vs Persistence summary | Same cell → last `print` block computing `delta` and `pct` |
...
1. **Cell 8 (Walk-forward evaluation)** in the same notebook. Same logic, just inside the loop.
2. **The plots in Cell 10.** Add a 4th bar group ("Persistence") to the RMSE/R² charts so the floor is visible. Add a second figure for log-return-space metrics + directional accuracy with a 50% reference line.
...
4. **`README.md`** — the "Test RMSE ~$622" claim should be reframed alongside the persistence baseline so reviewers see the full picture.
